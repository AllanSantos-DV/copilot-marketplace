...
| ------ | --------- | ------ |
| [`session-unloader`](./plugins/session-unloader) · [página](https://allansantos-dv.github.io/copilot-marketplace/p/session-unloader/) | Libera a RAM de sessões realmente ociosas do Copilot medindo inatividade contínua da árvore inteira de processos. Um daemon único é o dono do scan, da telemetria e do painel (dashboard) — cada sessão é só um cliente fino que aponta pra ele; nunca cria um servidor local próprio. Preserva voz, workers, mesas e recursos allowlisted; tempo e allowlist são configuráveis no canvas. Automático fail-closed e desligado por padrão, com dry-run e descarga manual pela tool unload_idle. | 0.7.0 |
| [`copilot-memory`](./plugins/copilot-memory) · [página](https://allansantos-dv.github.io/copilot-marketplace/p/copilot-memory/) | Memória persistente por projeto para o Copilot (pt-BR): guarda decisões, conhecimento e lições e as recupera por busca semântica, isoladas no projeto certo, com recall automático. Um curador destila os aprendizados da sessão em skills, e um grafo semântico do código (símbolos, chamadas, imports) ajuda a navegar repositórios grandes sem garimpar. Traz um painel de saúde, escopo e skills. | 1.17.0 |
| [`modo-auto`](./plugins/modo-auto) · [página](https://allansantos-dv.github.io/copilot-marketplace/p/modo-auto/) | Modo autonomo por sessao para o Copilot: uma MESA de agentes responde os ask_user e revisa os Stop, entao o agente nao para antes do plano terminar. Planeja em fases (ADR), constroi por TDD com revisao adversarial, audita reuso e seguranca, e um modo-sombra contesta a direcao em segundo plano. Nucleo hexagonal; cada modo e um interruptor da sessao. | 0.6.5 |
...
| [`mcp-bridge`](./plugins/mcp-bridge) · [página](https://allansantos-dv.github.io/copilot-marketplace/p/mcp-bridge/) | Restaura o MCP no GitHub Copilot onde ele é bloqueado por política mas as extensões são permitidas: conecta a servidores MCP (stdio/http/sse, com login OAuth) e expõe tools, prompts e resources nativamente ao agente. Aponta para um MOTOR local (ex.: mcp-gateway) só pelo nome — sem URL nem token no config. | 0.2.7 |
| [`canvas-sync`](./plugins/canvas-sync) · [página](https://allansantos-dv.github.io/copilot-marketplace/p/canvas-sync/) | Infra da vitrine: espelha as canvas extensions instaladas (installed-plugins, incluindo installs diretos) para ~/.copilot/extensions no SessionStart. Nunca rebaixa: compara a versao que entra com a do carimbo e recusa a mais velha, de forma visivel. Quando duas origens miram o mesmo canvas, vence a maior versao e a perdedora e reportada. Carimbo e log registram qual motor escreveu. | 0.6.0 |
<!-- plugins:end -->
