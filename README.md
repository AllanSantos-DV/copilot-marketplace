...
| [`modo-auto`](./plugins/modo-auto) · [página](https://allansantos-dv.github.io/copilot-marketplace/p/modo-auto/) | Modo autonomo por sessao para o Copilot: uma MESA de agentes responde os ask_user e revisa os Stop (nao deixa o agente parar antes do plano terminar), com base em plano vivo, memoria e pesquisa. Nucleo hexagonal (ports & adapters) com perfis plugaveis: modo-autonomo agora, modo-planejamento depois. | 0.2.58 |
...
| [`vox-engine`](./plugins/vox-engine) · [página](https://allansantos-dv.github.io/copilot-marketplace/p/vox-engine/) | Motor único de fala local para Windows (STT + TTS + tradução): um daemon via named pipe serve modelos residentes (Whisper) a vários apps e sessões, com GPU CUDA automática e sem admin. Sem uso, encerra sozinho e devolve RAM/VRAM ao sistema — volta em ~5s no próximo pedido, sem cortar captura em andamento. Distribuído por release assinado (Ed25519, fail-closed) e instalado/atualizado automaticamente pelos apps que o usam (voice-chat, visual-explainer). Não é um plugin instalável. | 0.22.7 |
<!-- plugins:end -->
