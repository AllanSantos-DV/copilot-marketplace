...
| ------ | --------- | ------ |
| [`copilot-memory`](./plugins/copilot-memory) · [página](https://allansantos-dv.github.io/copilot-marketplace/p/copilot-memory/) | Memória por projeto para o Copilot: descobre/sobe o daemon do servidor de memória (native-java, verificado por sha256); resolve o escopo pelo .memory/project.json ou git (worktree-safe) e migra a memória do escopo antigo com sua aprovação; injeta recall two-tier; um AGENTE CURADOR lê a conversa (checkpoints + turnos) e destila lições técnicas E comportamentais em skills; e GATES obrigatórios opt-in por projeto — ex.: revisor externo independente antes do git push, com recibo amarrado ao commit (bloqueio determinístico via hook). Painel (canvas) com saúde, escopo, skills e telemetria. | 1.8.1 |
| [`visual-explainer`](./plugins/visual-explainer) · [página](https://allansantos-dv.github.io/copilot-marketplace/p/visual-explainer/) | Explicações visuais animadas e interativas: peça um assunto e um agente desenhista gera uma página HTML autocontida (Canvas/SVG) com play/pause, zoom e peças clicáveis. Usa um kit de componentes (motor leve próprio + Konva para grafos) e escreve só uma spec — o builder monta o HTML otimizado. | 0.1.0 |
| [`voice-chat`](./plugins/voice-chat) · [página](https://allansantos-dv.github.io/copilot-marketplace/p/voice-chat/) | Converse por voz com o Copilot (pt-BR): fale e ouça um resumo falado. STT e TTS pelo motor único vox-engine, que a extensão instala, atualiza e usa sozinha no Windows — agora verificando a assinatura Ed25519 do instalador antes de executá-lo. | 1.5.21 |
...
| [`copilot-mobile`](./plugins/copilot-mobile) · [página](https://allansantos-dv.github.io/copilot-marketplace/p/copilot-mobile/) | Bridge do copilot-mobile: provisiona o daemon apartado na 1ª execução (baixa a release buildada, sem build), injeta o 🔊 (resumo falado) e detecta drift celular→PC na sessão viva. Pareamento, transporte e painel de configuração ficam no daemon (ícone da bandeja). | 0.3.3 |
