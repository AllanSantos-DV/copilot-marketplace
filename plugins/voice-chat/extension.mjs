...
function dispatchVoiceTurn(text, sidArg) {
    ...
    deliverTurnToFork(want, t, 0);
}

function deliverTurnToFork(sid, text, attempt) {
    const url = forks.get(sid);
    if (!url) {
        // The owner fork may not have (re-)registered yet — e.g. right after a
        // primary failover, forks re-register on a ~15s heartbeat. Retry briefly
        // instead of leaking the turn into the wrong session.
        if (attempt < 8) {
            setTimeout(() => deliverTurnToFork(sid, text, attempt + 1), 400);
            return;
        }
        dbg(`dispatchVoiceTurn: no fork url for sid=${sid} after retries — dropping (no cross-session leak)`);
        broadcastTo(sid, { type: "error", msg: "Não consegui rotear sua fala para esta sessão. Tente novamente." });
        return;
    }
    dbg(`dispatchVoiceTurn -> sid=${sid} url=${url} (attempt ${attempt})`);
    httpPostJson(url, "/inject", { text }).then((ok) => {
        if (!ok) {
            if (attempt < 8) {
                setTimeout(() => deliverTurnToFork(sid, text, attempt + 1), 400);
                return;
            }
            dbg(`dispatch to sid=${sid} failed after retries`);
            broadcastTo(sid, { type: "error", msg: "Falha ao enviar sua fala para esta sessão." });
        }
    });
}
