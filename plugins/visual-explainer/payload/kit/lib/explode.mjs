...
import { shapesBBox } from './geom.mjs';
...
export function buildExplodeScenes(spec){
  ...
  // bbox local das formas: agora em VXK.geom / kit/lib/geom.mjs (fonte única, importada acima).
  function localBox(L){                                                    // extensão local (relativa ao centro) da peça/placa
    if(Array.isArray(L.shapes) && L.shapes.length){ const s=(L.artScale!=null?L.artScale:1), b=shapesBBox(L.shapes,{minExtent:16,empty:[-40,-20,40,20],skipText:true}); return [b[0]*s,b[1]*s,b[2]*s,b[3]*s]; }
