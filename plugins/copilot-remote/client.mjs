...
function isAuthErr(e) {
    return /\b(401|403)\b|forbidden|unauthorized/i.test(String(e?.message || e));
}
...
// Open the SSE event stream. Parses `data: {json}\n\n` frames, skips comment
...
export function openEvents(baseUrl, token, { onEvent, onOpen, onError } = {}) {
    ...
    if (token) headers[TOKEN_HEADER] = token;
    try {
        req = mod(u).request(u, { method: "GET", headers }, (res) => {
